...
public class ControlText_SloveCaptcha {
    ...
    public boolean controlText() {
        if (text == null || text.isEmpty() || text.length() <= 3) {
            System.out.println("control text flase - value of text is null or <= 3");
            return false;
        }
        for (int i = 0; i < text.length(); i++) {
            if (!Character.isDigit(text.charAt(i))) {
                System.out.println("control text flase - value of text has char");
                return false;
            }
        }
        System.out.println("control text true");
        return true;
    }
}
